...
Для ускорения операций и повышения удобства работы, все дополнительные методы
в RightJS установлены на уровне прототипов, исключение составляют браузеры 
семейства Internet Explorer версий 6 и 7, которые не поддерживают подобной
возможности. Поэтому в случае если вы обращаетесь к элементу по ID или берете
элемент извне контекста библиотеки, то вам необходимо вызвать на этом элементе
метод `$()` хотя бы один раз, для того чтобы все расширения стали доступны.

Вам не нужно об этом беспокоиться если вы используете собственные методы  
RightJS для поиска элементов на странице. RightJS автоматически установит
все расширения, когда это необходимо.
...
удобной навигации по соседним элементам, например: {#parents}, {#siblings},
и т.д. Все эти методы могут быть использованы безусловно, как есть, а могут
принимть дополнительный аргумент-правило, соответственно которому результаты
...
Возвращает ссылку на родительский элемент. Если указано css-правило, то данный
...
Возвращает следующий по уровню элемент следующий за данным элементом. Если
...
Проверяет если данный элемент подпадает под указанное правило
...
Переключает наличие данного css-класс в списке классов данного элемента.
...
себя следующие ключи width, height, top, left, scrollLeft и scrollTop.
...
Данный элемент может получает любые стандартные опции класса {Xhr}.
